...
export default async function Home() {
  ...
  const catalog = [...programs].sort((a, b) => Number(b.active) - Number(a.active));
  return (
    <div className="bg-ink-950 text-white">
            ...
            <Reveal>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <ButtonLink href="#programs" size="lg">
                  Find your program <ArrowRight className="h-5 w-5" aria-hidden />
                </ButtonLink>
            ...
            >
              <div className="p-8 sm:p-12">
                <p className="text-xs font-black uppercase tracking-[0.25em] text-maroon">
                  Now enrolling · {programCopy(featured).partners ?? "Featured program"}
                </p>
                ...
                <div className="mt-6 flex flex-wrap gap-2 text-sm font-bold">
                  {[featured.cost_label, featured.hours_label, featured.duration_label].filter(Boolean).map((x) => (
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <Stagger className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              [Briefcase, "Employer pathways", "Every program is built with a university and a hiring employer, with interviews at the finish line."],
            ].map(([Icon, t, d]) => {
      ...
      <section id="partners" className="scroll-mt-20 border-t border-white/10 bg-ink py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {catalog.map((p) => {
              ...
              return (
                <Reveal key={p.id}>
                  <div className="flex h-full flex-col rounded-3xl border border-ink/10 p-8 transition hover:border-maroon/40 hover:shadow-xl">
                    <div className="flex items-start justify-between gap-3">
                      <p className="text-xs font-bold uppercase tracking-widest text-maroon">{c.partners ?? c.industry}</p>
                      <span
                    ...
                    <h3 className="mt-3 text-2xl font-black leading-tight">{p.name}</h3>
                    <p className="mt-2 flex-1 text-ink/60">{p.tagline}</p>
                    <div className="mt-5 flex flex-wrap gap-2 text-xs font-bold">
                      <span className="rounded-full bg-maroon/10 px-3 py-1 text-maroon">{c.industry}</span>
                      {p.active &&
                        [p.cost_label, p.hours_label].filter(Boolean).map((x) => (
                          <span key={x} className="rounded-full bg-ink/5 px-3 py-1 text-ink/70">
                            {x}
                          </span>
                        ))}
                    </div>
            ...
            <Reveal>
              ...
              >
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-ink/50">Universities &amp; employers</p>
                  <h3 className="mt-3 text-2xl font-black leading-tight">Launch the next program with us.</h3>
                  <p className="mt-2 text-ink/60">
                    Each {BRAND.name} program pairs a training partner with a hiring employer. Bring a role you need filled and we&apos;ll
                    build the pathway together.
                  </p>
                </div>
                <span className="mt-6 inline-flex items-center gap-2 font-black text-maroon">
                  Start a partnership <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" aria-hidden />
                </span>
              </a>
            </Reveal>
          </div>
